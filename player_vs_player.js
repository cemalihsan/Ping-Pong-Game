...
window.addEventListener('keydown', keyDownHandlerW);
...
function keyDownHandlerW(event) {
    switch (event.keyCode) {
        case 87:
            wUpPressed = true;
            break;
...
function collision(player, ball) {
    ...
    player.player2tom = player.y + player.height;
    player.left = player.x;

    ball.top = ball.y - ball.width / 2;
    ball.right = ball.x + ball.height / 2;
    ball.player2tom = ball.y + ball.width / 2;
    ball.left = ball.x - ball.height / 2;

    return ball.left < player.right && ball.top < player.player2tom && ball.right > player.left && ball.player2tom > player.top;
}
...
function update() {
    ...
    if (wUpPressed && player1.y > 0) {
        player1.y -= 10;
    } else if (sDownPressed && (player1.y < canvas.height - player1.height)) {
        player1.y += 10;
    }
...
function boardRender() {
    ...
    drawScore(3 * canvas.width / 4, canvas.height / 5, player2.score);
    drawStick(player1.x, player1.y, player1.width, player1.height, player1.color);
    drawStick(player2.x, player2.y, player2.width, player2.height, player2.color);
